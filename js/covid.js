...
function IndexChart(data, {
  ...
  colors = ['#cd2100','#1727da', '#05b805','#7005b8' ], // array of categorical colors
} = {}) {
  // Compute values.
...
document.addEventListener("DOMContentLoaded", async function() {
    ...
    chart  = IndexChart(covidData, {
      x: d => Date.parse(d.post_date),
      y: d => d.value + 0.1,
      z: d => d.type_of_covid_data,
      yLabel: "↑ Change in covid data (x)",
      width: 2000,
      height: 800
    })
    ...
    const chartDiv = document.getElementById('line-chart'); // Access the element
